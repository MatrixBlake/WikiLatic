...
//overall 4
RevisionSchema.statics.article_edited_by_largest_group_of_registered_users=function(admins,bots, callback){	
	return this.aggregate([
		{$match: {$and:[{user:{$nin:admins}},{user:{$nin:bots}},{anon:{"$exists":false}}]}},
	    {$group:{_id:{title:'$title', user:'$user'}, numberOfrevisions:{$sum:1}}},  
	    {$group:{_id:'$_id.title', numberOfresults:{$sum:1}}},   
...
//overall 5
RevisionSchema.statics.article_edited_by_smallest_group_of_registered_users=function(admins,bots, callback){	
	return this.aggregate([
		{$match: {$and:[{user:{$nin:admins}},{user:{$nin:bots}},{anon:{"$exists":false}}]}},
	    {$group:{_id:{title:'$title', user:'$user'}, numberOfrevisions:{$sum:1}}}, 
	    {$group:{_id:'$_id.title', numberOfresults:{$sum:1}}},      
