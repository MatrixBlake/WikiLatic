		...
		if(req.url=="/"){
			...
			Revision.highest_number_of_revisions(function(err,result){
				if (err){
					...
					console.log(result);
					o1_result=result;									
					Revision.article_edited_by_largest_group_of_registered_users(req.app.locals.admins,req.app.locals.bots,function(err,result){
						if (err){
							console.log("Cannot find lowest number of revisions!")
						}else{
							console.log(result);
							o4_result=result;
							Revision.article_edited_by_smallest_group_of_registered_users(req.app.locals.admins,req.app.locals.bots,function(err,result){
								if (err){
									console.log("Cannot find lowest number of revisions!")
								}else{
									console.log(result);
